...
| name | type | description | required |
|---|:---:|:---:|:---:|
| title | String | 소통 플랫폼 게시물 제목 | O |
| contents | String | 소통 플랫폼 게시물 내용 | O |

###### Example

```bash
curl -v -X POST "http://localhost:4200/api/v1/communication/" \
 -H "Authorization: Bearer {JWT}" \
 -d "title={title}" \
 -d "contents={contents}
```
...
```bash
curl -v -X GET "http://localhost:4200/api/v1/communication/list" \
 -H "Authorization: Bearer {JWT}"
```
...
| code | String | 결과 코드 | O |
| message | String | 결과 메세지 | O |
| communicationList | communicationListItem[] | 소통 플랫폼 게시물 리스트 | O |

**communicationListItem**
| name | type | description | required |
|---|:---:|:---:|:---:|
| communicationNumber | int | 접수 번호 | O |
| title | String | 제목 | O |
| writerId | String | 작성자 아이디</br>(첫글자를 제외한 나머지 문자는 *) | O |
| writeDatetime | String | 작성일</br>(yy.mm.dd 형태) | O |
| viewCount | int | 조회수 | O |
...
- method : **GET**  
- URL : **/communication/list/{searchWord}**  
...
```bash
curl -v -X GET "http://localhost:4200/api/v1/communication/list/${searchWord}" \
 -H "Authorization: Bearer {JWT}"
```
...
- method : **GET**  
- URL : **/communication/{communicationNumber}**  
...
```bash
curl -v -X GET "http://localhost:4200/api/v1/communication/${communicationNumber}" \
 -H "Authorization: Bearer {JWT}"
```
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 접수번호를 입력받고 요청을 보내면 해당하는 디자이너 게시물 데이터를 반환합니다. 만약 불러오기에 실패하면 실패처리를 합니다. 인가 실패, 권한 없음, 데이터베이스 에러가 발생할 수 있습니다.

- method : **GET**  
- URL : **/communication/secret/{communicationNumber}**  
...
```bash
curl -v -X GET "http://localhost:4200/api/v1/communication/secret/${communicationNumber}" \
 -H "Authorization: Bearer {JWT}"
```
...
{
  "code": "PD",
  "message": "Permission Denied: Only authors, designers, and administrators can access comments of private posts."
}
```
